...
column_order <- c("l_npp_gpp_50", "l_cveg_star_npp_50", "l_croot_cveg_50", "l_cwood_cveg_50", "l_cleaf_cveg_star_50")
...
# By model
df_long <- df %>%
  select(modl, any_of(column_order)) |> 
...
df_long <- df %>%
  group_by(cn_coupled) |> 
  summarise(across(all_of(column_order), ~ median(.x, na.rm = TRUE))) |> 
...
heatmaps <- cowplot::plot_grid(
  ...
  align = "hv",    # vertical alignment
  axis = "lr",    # align left and right margins (y-axis)
  rel_heights = c(1, 0.189, 0.189, 0.135, 0.15),
  labels = letters[1:4]
)

heatmaps

ggsave(here("fig/summary_tablev20250825.pdf"), width = 7, height = 9)
```
